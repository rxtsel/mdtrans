...
- Never write to the input file. By default, open a temporary `<name>-<language>-translate.md` in `$EDITOR`; `--stdout` prints Markdown only. Diagnostics go to stderr. Never log keys or documents.
...
- Keep dependencies small. Do not add caching, complex ASTs, streaming, TUIs, or infrastructure beyond the MVP.
